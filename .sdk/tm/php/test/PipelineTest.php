...
{
    private static function utility(): BranchioUtility
    {
        return new BranchioUtility();
    }
    ...
    private static function ctx(array $over = []): BranchioContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BranchioContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BranchioOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BranchioError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BranchioOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BranchioMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BranchioOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BranchioMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BranchioOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BranchioMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BranchioSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BranchioMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BranchioResponse([]),
            'result' => new BranchioResult([]),
        ]);
        [, $err] = BranchioMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BranchioSpec([]),
            'response' => null,
            'result' => new BranchioResult([]),
        ]);
        [, $err] = BranchioMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BranchioSpec(['step' => 's']),
            'response' => new BranchioResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BranchioResult([]),
        ]);
        [, $err] = BranchioMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BranchioSpec(['step' => 's']),
            'response' => new BranchioResponse(self::resp(200, ['v' => 1])),
            'result' => new BranchioResult([]),
        ]);
        [, $err] = BranchioMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BranchioSpec(['step' => 's']),
            'response' => new BranchioResponse(self::resp(200, ['v' => 2])),
            'result' => new BranchioResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BranchioMakeResponse::call($ctx);
    ...
    {
        $preset = new BranchioResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BranchioSpec([]),
            'response' => new BranchioResponse([]),
            'result' => new BranchioResult([]),
        ...
        [$got, $err] = BranchioMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BranchioResult([])]);
        [, $err] = BranchioMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BranchioSpec([]), 'result' => null]);
        [, $err] = BranchioMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BranchioSpec(['step' => 's']),
            'result' => new BranchioResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BranchioOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BranchioMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BranchioSpec(['step' => 's']),
            'result' => new BranchioResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BranchioOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BranchioMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BranchioResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BranchioSpec([]),
            'result' => new BranchioResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BranchioMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BranchioMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BranchioError('boom', 'boom');
        $utility->fetcher = function (BranchioContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BranchioSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BranchioMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BranchioContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BranchioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BranchioResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BranchioContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BranchioSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BranchioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BranchioResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BranchioSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BranchioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BranchioMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BranchioMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BranchioResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BranchioDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BranchioResult(['ok' => false])]);
        $this->expectException(BranchioError::class);
        BranchioDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BranchioResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BranchioMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BranchioMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BranchioBaseFeature();
        $b = new BranchioBaseFeature();
        BranchioFeatureAdd::call($ctx, $a);
        BranchioFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BranchioBaseFeature
    {
        $f = new BranchioBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BranchioFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BranchioContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BranchioSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BranchioPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BranchioResponse(['headers' => null]),
            'result' => new BranchioResult([]),
        ]);
        BranchioResultHeaders::call($ctx);
