...
class BranchioSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = BranchioHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = BranchioHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!BranchioFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, BranchioFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return BranchioUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = BranchioHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = BranchioHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = BranchioHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new BranchioError($op . "_allow",
                "BranchioSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = BranchioHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new BranchioError("graphql_error",
                "BranchioSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
