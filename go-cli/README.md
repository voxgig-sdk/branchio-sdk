...
boru-driven command-line client **and** interactive REPL for the Branchio
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branchio-cli)
make build
...
BRANCHIO_BASE=https://api.example.com ./branchio-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branchio-cli
   ```
...
Run with no arguments to open a REPL (prompt `branchio>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `url`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/branchio-cli`. |
...
(`./branchio-cli <expr>`) and interactively (the REPL), and expressions compose the
