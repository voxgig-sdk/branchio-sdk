    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BRANCHIO_TEST_URL_ENTID JSON to run live")
        client = setup["client"]
        ...
        assert url_ref01_data["id"] is not None
...
def _url_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BRANCHIO_TEST_URL_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("BRANCHIO_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = BranchioSDK(helpers.to_map(merged_opts))

    _live = env.get("BRANCHIO_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BRANCHIO_TEST_EXPLAIN") == "TRUE",
        "live": _live,
