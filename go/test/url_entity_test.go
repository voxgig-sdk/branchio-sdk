...
func TestUrlEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BRANCHIO_TEST_URL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func urlBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BRANCHIO_TEST_URL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BRANCHIO_TEST_URL_ENTID"])
	...
	live := env["BRANCHIO_TEST_LIVE"] == "TRUE"
	return &entityTestSetup{
		client:        client,
		data:          entityData,
		idmap:         idmapResolved,
		env:           env,
		explain:       env["BRANCHIO_TEST_EXPLAIN"] == "TRUE",
		live:          live,
