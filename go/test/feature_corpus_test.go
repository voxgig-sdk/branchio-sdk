...
//
// NewBranchioSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.BranchioSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewBranchioSDK(opts)
}
...
func fcCandidates(client *sdk.BranchioSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.BranchioSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.BranchioSDK, name string) any {
