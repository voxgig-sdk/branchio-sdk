...
import (
	...
	omni "github.com/voxgig-sdk/branchio-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BranchioError); is {
				out := map[string]any{
					"name":    "BranchioError",
...
func omniCtx(arg any, client *sdk.BranchioSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BranchioSDK); is {
			client = live
