...
class FtCtrl extends BranchioControl
{
...
class FtClient
{
    ...
    private BranchioUtility $utility;

    public function __construct(BranchioUtility $utility, array $options)
    {
    ...
    public function get_utility(): BranchioUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BranchioContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BranchioUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BranchioContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BranchioConfig::make_feature($name);
    ...
    private static function buildUrl(BranchioSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BranchioError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BranchioResponse($fetched);
        }
    ...
    private function fail(BranchioContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BranchioContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
