...
const TEST_SEED = {"entity":{"url":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+BranchioSDK\b/.test(code) ||
    /\bBranchioSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BranchioSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BranchioSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
