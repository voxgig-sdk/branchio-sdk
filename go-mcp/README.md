...
[MCP](https://modelcontextprotocol.io) server exposing the Branchio SDK as
two agent tools — `branchio_list` and `branchio_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branchio-mcp)
make build

# 2. Provide credentials via the environment
export BRANCHIO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user branchio \
  -- /absolute/path/to/branchio-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./branchio-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// branchio_list: first page of records
{ "entity": "url" }
{ "entity": "url", "query": { } }

// branchio_load: one record by id
{ "entity": "url", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branchio-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user branchio \
     -- "$PWD"/dist/*/branchio-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `branchio_list` and `branchio_load` tools now appear
   in new sessions. Ask the agent to *"list url using branchio"*
   and it calls `branchio_list` with `{"entity":"url"}`.
...
```sh
export BRANCHIO_APIKEY=sk_live_xxx            # API key
export BRANCHIO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `branchio_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `branchio_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRANCHIO_APIKEY` | API key sent with every request. |
| `BRANCHIO_BASE` | Optional override of the API base URL. |
...
```sh
./branchio-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"branchio_load","arguments":{"entity":"url","query":{"id":1}}}}'
```
