...
// Branchio SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class BranchioMakePoint
{
    public static function call(BranchioContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof BranchioError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = BranchioHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = BranchioHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
