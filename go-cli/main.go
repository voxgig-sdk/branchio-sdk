// Command Branchio-cli is a CLI + REPL for the Branchio SDK,
...
const prompt = "branchio"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "url"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BRANCHIO_APIKEY (api key), BRANCHIO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BRANCHIO_APIKEY carries the API key and
	// BRANCHIO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BRANCHIO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BRANCHIO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBranchioSDK(opts)
