...
class BranchioMakeError
{
    public static function call(?BranchioContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BranchioContext([], null);
        }
        $op = $ctx->op ?? new BranchioOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BranchioResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BranchioError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BranchioError('', $msg, $ctx);
        ...
        if ($err instanceof BranchioError) {
