...
local BranchioSDK = {}
BranchioSDK.__index = BranchioSDK
...
BranchioSDK._make_feature = _make_feature


function BranchioSDK.new(options)
  local self = setmetatable({}, BranchioSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BranchioSDK:options_map()
...
function BranchioSDK:get_utility()
  return Utility.copy(self._utility)
end


function BranchioSDK:get_root_ctx()
  return self._rootctx
end


function BranchioSDK:prepare(fetchargs)
...
function BranchioSDK:direct(fetchargs)
...
-- by passing it.
function BranchioSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BranchioSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BranchioSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BranchioSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BranchioSDK.new(sdkopts)
  sdk.mode = "test"
